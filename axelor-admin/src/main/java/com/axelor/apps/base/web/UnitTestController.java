...
public class UnitTestController {
  ...
  public void importTests(ActionRequest request, ActionResponse response) {
    try {
      ...
      response.setFlash(I18n.get(AdminExceptionMessage.UNIT_TEST_IMPORT_SUCCESS));
  ...
  public void exportTests(ActionRequest request, ActionResponse response) {
    try {
      ...
      if (ObjectUtils.isEmpty(idsList)) {
        response.setError(I18n.get(AdminExceptionMessage.UNIT_TEST_EXPORT_RECORD_NOT_SPECIFIED));
        return;
      }

...
public class UnitTestLineController {
  ...
  public void validateOnChange(ActionRequest request, ActionResponse response) {
    ...
    if (unitTestLineService.isTargetUndefined(test, testLine)) {
      uniqueErrors.put(
          TEST_LINE_TARGET, I18n.get(AdminExceptionMessage.UNIT_TEST_TARGET_VAR_NOT_SPECIFIED));
    } else if (isTargetValidForAdd(target, actionTypeSelect)) {
      uniqueErrors.put(
          TEST_LINE_TARGET, I18n.get(AdminExceptionMessage.UNIT_TEST_INVALID_TARGET_FOR_ADD));
    }
    if (unitTestLineService.isValidAction(testLine)) {
      uniqueErrors.put(
          TEST_LINE_VALUE, I18n.get(AdminExceptionMessage.UNIT_TEST_INVALID_ACTION_NAME));
    } else if (isValidModelName(actionTypeSelect, value)) {
      uniqueErrors.put(
          TEST_LINE_VALUE,
          String.format(I18n.get(AdminExceptionMessage.UNIT_TEST_INVALID_MODEL_NAME), value));
    } else if (isValidSelectValueFormat(actionTypeSelect, value)) {
      uniqueErrors.put(
          TEST_LINE_VALUE,
          String.format(I18n.get(AdminExceptionMessage.UNIT_TEST_INVALID_SELECT_FORMAT), value));
    }
